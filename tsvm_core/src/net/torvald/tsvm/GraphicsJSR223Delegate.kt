...
class GraphicsJSR223Delegate(val vm: VM) {
    ...
    private val fsTerms = floatArrayOf(
        7f/16f,
    ...
    fun imageToDisplayableFormat(srcPtr: Int, destPtr: Int, width: Int, height: Int, channels: Int, useDither: Boolean) {
        ...
        if (useDither) {
            ...
            for (k in 0L until len) {
                ...
                val or = srcimg.getFloat(channels * k)
                val og = srcimg.getFloat(channels * k + 1)
                val ob = srcimg.getFloat(channels * k + 2)
                val oa = if (useAlpha) srcimg.getFloat(channels * k + 3) else 1f

                val oqr = or.coerceIn(0f, 1f).times(15).roundToInt()
                val oqg = og.coerceIn(0f, 1f).times(15).roundToInt()
                val oqb = ob.coerceIn(0f, 1f).times(15).roundToInt()
                ...
                offsets.forEachIndexed { index, offset ->
                    ...
                    if (px in 0 until width && py in 0 until height) {
                        ...
                        val srca = srcimg.getFloat(channels * offset + 3)
                        srcimg.setFloat(channels * offset + 0, srcr + qer * fsTerms[index])
                        srcimg.setFloat(channels * offset + 1, srcg + qeg * fsTerms[index])
                        srcimg.setFloat(channels * offset + 2, srcb + qeb * fsTerms[index])
                        if (useAlpha) srcimg.setFloat(channels * offset + 3, srca + qea * fsTerms[index])
                    }
                }
            }
