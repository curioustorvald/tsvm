...
class GraphicsJSR223Delegate(val vm: VM) {
    ...
    private val stuckiTerms = floatArrayOf(
        8f/42f,4f/42f,
     ...
     */
    fun imageToDisplayableFormat(srcPtr: Int, destPtr: Int, width: Int, height: Int, channels: Int, useDither: Boolean) {
        val sign = if (destPtr >= 0) 1 else -1
        val len = width * height
        if (useDither) {
            val srcimg = UnsafeHelper.allocate(width * height * 4L * channels) // array of floats!
            // we won't pre-fill the srcimg, it'll be filled during the dithering process

            srcimg.setFloat(0, vm.peek(srcPtr + 0L)!!.toUint().toFloat())
            srcimg.setFloat(4, vm.peek(srcPtr + 1L)!!.toUint().toFloat())
            srcimg.setFloat(8, vm.peek(srcPtr + 2L)!!.toUint().toFloat())
            srcimg.setFloat(12, vm.peek(srcPtr + 3L)!!.toUint().toFloat())

            for (k in 0L until len) {

                val or = srcimg.getFloat(4*k)
                val og = srcimg.getFloat(4*k + 1)
                val ob = srcimg.getFloat(4*k + 2)
                val oa = srcimg.getFloat(4*k + 3)

                val oqr = or.times(15).roundToInt()
                val oqg = og.times(15).roundToInt()
                val oqb = ob.times(15).roundToInt()
                val oqa = oa.times(15).roundToInt()

                val nc = if (oa < 128) 255.toByte() else clut[oqr*256 + oqg*16 + oqb]
                val (nr, ng, nb, na) = GraphicsAdapter.DEFAULT_PALETTE_NUMBERS[nc.toUint()]
                ...
                val offsets = longArrayOf(k+1, k+2,
                    k+width-2,k+width-1,k+width,k+width+1,k+width+2,
                    k+2*width-2,k+2*width-1,k+2*width,k+2*width+1,k+2*width+2,
                )
                ...
                offsets.forEachIndexed { index, offset ->
                    ...
                    if (px in 0 until width && py in 0 until height) {
                        val srcr = vm.peek(srcPtr + channels * offset)!!.toUint().toFloat().div(255f)
                        val srcg = vm.peek(srcPtr + channels * offset + 1)!!.toUint().toFloat().div(255f)
                        val srcb = vm.peek(srcPtr + channels * offset + 2)!!.toUint().toFloat().div(255f)
                        val srca = vm.peek(srcPtr + channels * offset + 3)!!.toUint().toFloat().div(255f)
                        srcimg.setFloat(4 * channels * offset, srcr + qer * stuckiTerms[index])
                        srcimg.setFloat(4 * channels * offset + 1, srcg + qeg * stuckiTerms[index])
                        srcimg.setFloat(4 * channels * offset + 2, srcb + qeb * stuckiTerms[index])
                        srcimg.setFloat(4 * channels * offset + 3, srca + qea * stuckiTerms[index])
                    }
                }
            }
        }
