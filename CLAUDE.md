...
**S $Dxny's `$n` follow-up action (backported from the web engine 2026-08-01) fires even on a note-less row.** `applySEffect`'s case `0xD` is still a no-op by design — the whole command lives in `applyTrackerRow`/`applyTrackerTick`. `scheduleDxnyAction` arms `voice.noteActionTick`/`delayedAction` at absolute tick `$x+$y` whenever `$y` is nonzero; it is called from all SIX note-event branches, including `row.note == 0x0000` — a row with no note of its own still lets `$n` act on whatever the channel is already sounding (this is what makes FastTracker `Kxx`, which has no note-column entry, expressible as `S $D00xx`; see `TAUD_CONVERSION_NOTES.md` §4.6). `applyTrackerTick`'s early-continue guard must check `noteActionTick >= 0` too, or an otherwise-idle voice with only an armed action gets skipped before it fires. `$n=4` "Key lift" is `forceKeyLift` — the same `jumpToSustainEnd` core as `applyKeyLift` but bypassing the instrument's own Key-Lift flag, a per-note override in the same spirit as `S $73..$76`. Scenario coverage: `devtests/webconf/EngineBackportTest.java` legs 6a/6b.

**Overlay-compiling just `AudioAdapter.kt` against a stale `out/production/tsvm_core` needs `-module-name tsvm_core`, and can still fail on unrelated staleness.** Kotlin mangles `internal` accessors with the compiling module's name (`getInstruments$tsvm_core()`); omitting `-module-name tsvm_core` from the overlay `kotlinc` invocation silently mangles to `$main` instead, which throws `NoSuchMethodError` from any pre-compiled sibling file (e.g. `AudioJSR223Delegate.class`) that calls the real accessor — this bit the 2026-08-01 backport and is now folded into the documented recipe (`devtests/ixmp/README.md`). Separately: if `out/production` predates OTHER unrelated source changes (e.g. a `TaudInstPatch` field added since the last IntelliJ build), a one-file overlay can't bridge that gap — a Java caller in a stale sibling `.class` will throw `NoSuchMethodError` on a constructor call that no longer matches, reproducibly, even against a byte-for-byte pristine `AudioAdapter.kt`. The fix is to compile the WHOLE module fresh instead of overlaying: `kotlinc -jvm-target 17 -module-name tsvm_core -cp <lib jars + kotlin-stdlib 1.8.21> $(find tsvm_core/src -name '*.kt') -d <outdir>` — no `-Xfriend-paths` needed for a full build, and it only takes ~10s for the whole 55-file module.
