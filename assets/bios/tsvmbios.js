...
let o=[];for(let y=0;y<164;y++)o[y]=(Math.round(Math.random()*56)-28)|0;
for(let p=0;p<=28;p++){for(let y=0;y<164;y++){for(let x=0;x<560;x++)
{let f=y*560+x;let d=x+10*o[y];if(d>=0&&d<560)graphics.plotPixel(d,95+y,logo[f]);}
if(o[y]!=0)o[y]-=Math.sign(o[y]);}}

// display logo in mundane way
//for(let p=164;p>=0;p-=2){for(let y=0;y<164;y++){for(let x=0;x<560;x++)
//{let f=(y-p)*560+x;graphics.plotPixel(x,95+y,(f>0)?logo[f]:255);}}}
...
let time = sys.nanoTime();
while (sys.nanoTime() - time < 3221225472) sys.spin();
// clear screen
